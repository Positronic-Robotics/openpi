...
from openpi.policies import positronic_policy
...
@dataclasses.dataclass(frozen=True)
class LeRobotPositronicDataConfig(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[
                positronic_policy.PositronicInputs(
                    action_dim=model_config.action_dim, model_type=model_config.model_type
                )
            ],
            outputs=[positronic_policy.PositronicOutputs()],
        )
...
_CONFIGS = [
    ...
    # DROID in absolute joint-position action space (the RoboLab benchmark policies). The model predicts
    # joint deltas; AbsoluteActions adds the observed joint position back, so the served chunk is absolute
    # joint positions. Checkpoint: gs://openpi-assets-simeval/pi05_droid_jointpos.
    TrainConfig(
