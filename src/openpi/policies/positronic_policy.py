...
def make_positronic_example() -> dict:
    """Creates a random input example for the Positronic policy."""
    return {
        "observation/state": np.random.rand(7),
        "observation/image": np.random.randint(256, size=(224, 224, 3), dtype=np.uint8),
        "observation/wrist_image": np.random.randint(256, size=(224, 224, 3), dtype=np.uint8),
        "prompt": "pick up the green cube and put it on top of the red cube",
    }
...
class PositronicInputs(transforms.DataTransformFn):
    """
...
@dataclasses.dataclass(frozen=True)
class PositronicOutputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
